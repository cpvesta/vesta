...
# Vesta CP Debian installer v 0.9
...
if [ "$release" -eq 13 ]; then
    ...
    echo "Vesta is much more faster with Debian 10 ."
...
help() {
  ...
  Example: bash $0 -e demo@cpvesta.ru -p p4ssw0rd --apache no --phpfpm yes"
    exit 1
}
...
wget -q "apt.cpvesta.ru/deb_signing.key" -O /dev/null
...
echo "=== Installing Vesta repo"
# echo "deb http://$RHOST/$codename/ $codename vesta" > $apt/vesta.list
# wget $CHOST/deb_signing.key -O deb_signing.key
# apt-key add deb_signing.key
echo "deb [arch=$ARCH signed-by=/usr/share/keyrings/vesta-keyring.gpg] https://$RHOST/$codename/ $codename vesta" > $apt/vesta.list
curl -s $CHOST/deb_signing.key | gpg --dearmor | tee /usr/share/keyrings/vesta-keyring.gpg > /dev/null 2>&1
...
echo "== Copying logrotate for Vesta logs"
...
if [ "$mysql" = 'yes' ] || [ "$mysql8" = 'yes' ]; then
    ...
    if [ "$release" -eq 10 ]; then
      ...
      wget -nv -O /root/phpmyadmin/pma.sh http://c.cpvesta.ru/debian/10/pma/pma.sh 
      wget -nv -O /root/phpmyadmin/create_tables.sql http://c.cpvesta.ru/debian/10/pma/create_tables.sql
  ...
  if [ "$release" -gt 10 ]; then
      ...
      mkdir /root/phpmyadmin
      wget -nv -O /root/phpmyadmin/pma.sh http://c.cpvesta.ru/debian/11/pma/pma.sh 
      wget -nv -O /root/phpmyadmin/create_tables.sql http://c.cpvesta.ru/debian/11/pma/create_tables.sql
      bash /root/phpmyadmin/pma.sh
      blowfish=$(gen_pass)
      echo "\$cfg['blowfish_secret'] = '$blowfish';" >> /etc/phpmyadmin/config.inc.php
...
if [ "$clamd" = 'yes' ]; then
 ...
 if [ ! -d "/var/run/clamav" ]; then
    ...
    if [ -d "$clamavfolder" ]; then
        ...
        wget -nv -O $clamavfolder/foxhole_all.cdb http://c.cpvesta.ru/tools/clamav/foxhole_all.cdb
...
if [ "$spamd" = 'yes' ]; then
    ...
    echo "== Adding Vesta rules to SpamAssassin"
    cat <<EOF > /etc/spamassassin/vesta.cf
...
EOF
    ...
    wget -nv -O /etc/spamassassin/barracuda.cf http://c.cpvesta.ru/tools/spamassassin/barracuda.cf
    ensure_startup $currentservice
    systemctl restart $currentservice
fi
...
echo "== Get public ip"
pub_ip=$(curl -4 -s https://scripts.cpvesta.ru/ip.php)
...
# wget -nv https://c.cpvesta.ru/tools/patches/rcube_vcard.patch -O /root/rcube_vcard.patch
