...
if [ "$type" = "ubuntu" ]; then
    ...
    echo "This Vesta CP version supports only Debian."
    exit;
fi
if [ "$type" = "amazon" ]; then
...
if [ -e '/usr/bin/wget' ]; then
    wget http://c.cpvesta.ru/vst-install-$type.sh -O vst-install-$type.sh
    if [ "$?" -eq '0' ]; then
...
if [ -e '/usr/bin/curl' ]; then
    curl -O http://c.cpvesta.ru/vst-install-$type.sh
    if [ "$?" -eq '0' ]; then
        bash vst-install-$type.sh $*
        exit
    else
