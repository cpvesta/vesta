...
MAINTAINER_EMAIL='support@cpvesta.ru'
...
GIT_SRC='https://raw.githubusercontent.com/cpvesta/vesta/master/src'
GIT_REP="$GIT_REP/deb"

C_WEB_ADDRESS="c.cpvesta.ru"
WWW_FOLDER="/var/www"
...
APT_WEB_ADDRESS="apt.cpvesta.ru"
PATH_OF_APT_REPO_ROOT="$WWW_FOLDER/$APT_WEB_ADDRESS/html"
PATH_OF_APT_REPO="$PATH_OF_APT_REPO_ROOT/$TARGET_DEB_NAME"

VESTA_VER=$(curl -s https://raw.githubusercontent.com/cpvesta/vesta/master/src/deb/latest.txt)
...
if [ "$VESTAGIT_B" = true ]; then
  ...
  cd /root
  rm -rf vesta/
  git clone https://github.com/cpvesta/vesta.git
...
Label: vesta apt repository
Codename: $TARGET_DEB_NAME
Architectures: amd64 source
Components: vesta
Description: vesta debian package repo
SignWith: yes
